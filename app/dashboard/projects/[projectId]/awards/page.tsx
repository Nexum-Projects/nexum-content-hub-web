...
import { fetchAwardsPage } from "@/app/actions/content/fetch-paginated-lists";
import getProjectSummary from "@/app/actions/content/get-project-summary";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { AwardsListClient } from "@/components/project-lists/awards-list-client";
...
export default async function AwardsPage({
  ...
  const [projectRes, listRes] = await Promise.all([
    ...
    fetchAwardsPage(projectId, sp),
  ]);

  const project = projectRes.status === "success" ? projectRes.data : fallbackProjects[0];
  const awards = listRes.status === "success" ? listRes.data.items : [];
  ...
  const createHref = `/dashboard/projects/${projectId}/awards/new`;
  const orderHref = `/dashboard/projects/${projectId}/awards/order`;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <section className="flex flex-col gap-4 rounded-xl border border-border bg-card p-6 shadow-sm sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">Logros / Premios</h1>
          <p className="mt-1 text-sm text-muted-foreground">Reconocimientos publicados para {project.name}.</p>
