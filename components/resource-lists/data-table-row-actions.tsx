...
function Item({ children, disabled, isLoading, variant, ...props }: PropsWithChildren<ItemProps>) {
  if (props.type === "link") {
  ...
  return (
    <DropdownMenuItem disabled={disabled || isLoading} variant={variant} onClick={onClick}>
      {children}
    </DropdownMenuItem>
  );
}
